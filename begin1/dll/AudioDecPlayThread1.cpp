...
unsigned WINAPI DepackandAMRDecodeThread1(void* pvParam)
{
	...
	while(1)
	{
		static BOOL pRecvfromBufferQueue_1Empty;
		critical_section3_1.Lock();
		pRecvfromBufferQueue_1Empty = pRecvfromBufferQueue_1.empty();
		critical_section3_1.Unlock();

		if(!pRecvfromBufferQueue_1Empty)//接收队列非空则取数据进行拆包、解码工作
		{	
			critical_section3_1.Lock();
			AMRDecBuf = pRecvfromBufferQueue_1.front();//从队列取出一帧
			critical_section3_1.Unlock();
			...
			{	
				...
				dwRecDataLength+=328;
				if(dwRecDataLength>328*10)//大概每100帧保存一次，不能设置过大或过小
			...
			dwPCMForAudioOutQueueSize=pPCMForAudioOutQueue_1.size();
			critical_section4_1.Unlock();

			if(dwPCMForAudioOutQueueSize<50)     //缓冲队列长度为50帧，超过50帧后的数据丢失
			{
				critical_section4_1.Lock();
				pPCMForAudioOutQueue_1.push(AudioDecBuf);       //解码完毕的PCM数据送入队列
				critical_section4_1.Unlock();
			}
			else
			{
			...
			critical_section3_1.Lock();
			pRecvfromBufferQueue_1.pop();			//销毁已经拆包、解码处理的帧
			critical_section3_1.Unlock();
			...
			//					CopyMemory(pRecPCMFileBuffer+dwRecDataLength,&AudioDecBuf.sequencenum,328);
			//					dwRecDataLength+=328;
			//					if(dwRecDataLength>328*100)//大概每帧保存一次，不能设置过大或过小
			//						//过大会造成不同步；过小造成写文件操作频繁，暂用CPU资源
			//						//具体数值实测决定
			//					{
			//						PCMRecSuccessFlag=TRUE;
			...
			//
			//				if(dwPCMForAudioOutQueueSize<50)     //缓冲队列长度为帧，超过帧后的数据丢失
			//				{
			//					critical_section4_2.Lock();
