                               ...
                               uint8_t **ref_picture)
{
    ...
    if(s->flags&CODEC_FLAG_EMU_EDGE){
        if(   src_x >= s->h_edge_pos - 17
           || src_y >= s->v_edge_pos - 17){
    ...
    if(s->flags&CODEC_FLAG_EMU_EDGE){
        if(   src_x >= (s->h_edge_pos>>1) - 9
           || src_y >= (s->v_edge_pos>>1) - 9){
                                  ...
                                  int motion_x, int motion_y)
{
    ...
    if(s->unrestricted_mv && (s->flags&CODEC_FLAG_EMU_EDGE)){
        if(   src_x > h_edge_pos - (motion_x&1) - w
           || src_y > v_edge_pos - (motion_y&1) - h){
...
if(s->quarter_sample)
{
    ...
    if(   src_x > s->h_edge_pos - (motion_x&1) - 16
       || src_y >    v_edge_pos - (motion_y&1) - h){
                               ...
                               int motion_x, int motion_y, int h)
{
    ...
    if(   src_x > s->h_edge_pos - (motion_x&3) - 16
       || src_y >    v_edge_pos - (motion_y&3) - h  ){
        ff_emulated_edge_mc(s->edge_emu_buffer, ptr_y, s->linesize,
...
static inline void chroma_4mv_motion(MpegEncContext *s,
    ...
    ptr = ref_picture[1] + offset;
    if(s->flags&CODEC_FLAG_EMU_EDGE){
        if(   src_x > (s->h_edge_pos>>1) - (dxy &1) - 8
           || src_y > (s->v_edge_pos>>1) - (dxy>>1) - 8){
