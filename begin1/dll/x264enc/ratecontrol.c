...
///    x264_cpu_restore( h->param.cpu );
             ...
             h->param.rc.i_vbv_buffer_size > 0 )
    {
        if( h->param.rc.i_vbv_buffer_size < 3 * h->param.rc.i_vbv_max_bitrate / rc->fps ) {
            h->param.rc.i_vbv_buffer_size = (int)(3 * h->param.rc.i_vbv_max_bitrate / rc->fps);
...
#define ABR_INIT_QP ( h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.i_rf_constant : 24 )
    ...
    rc->qp_constant[SLICE_TYPE_I] = x264_clip3( (int)(h->param.rc.i_qp_constant - rc->ip_offset + 0.5), 0, 51 );
    rc->qp_constant[SLICE_TYPE_B] = x264_clip3( (int)(h->param.rc.i_qp_constant + rc->pb_offset + 0.5), 0, 51 );
...
///    rc->lstep = exp2f(h->param.rc.i_qp_step / 6.0);
    ...
    if( h->param.rc.b_stat_read )
    {
        ...
        for(i=0; i<rc->num_entries; i++){
            ...
            rce->qscale = rce->new_qscale = (float)qp2qscale(20);
...
///    x264_cpu_restore( h->param.cpu );
    ...
    else
    {
        ...
        if( y < rc->last_row && h->stat.i_slice_count[rc->slice_type] > 0 )
        {
            ...
            int b0 = (int)predict_row_size_sum( h, y, rc->qpm );
            int b1 = b0;
            ...
            {
                rc->qpm ++;
                b1 = (int)predict_row_size_sum( h, y, rc->qpm );
            }
...
static double get_diff_limited_q(x264_t *h, ratecontrol_entry_t *rce, double q)
{
    ...
    {
        float mask = 1 - (float)pow( (float)(rce->i_count / rcc->nmb), 2 );
...
static void update_vbv( x264_t *h, int bits )
{
    ...
    rcc->buffer_fill = x264_clip3( (int)rcc->buffer_fill, 0, (int)rcc->buffer_size );
}
...
static double clip_qscale( x264_t *h, int pict_type, double q )
{
          ...
          ( pict_type == SLICE_TYPE_I && rcc->last_non_b_pict_type == SLICE_TYPE_I ) ) )
    {
        if( rcc->buffer_fill/rcc->buffer_size < 0.5 )
            q /= (double)x264_clip3f( (float)( 2.0*rcc->buffer_fill/rcc->buffer_size ), 0.5, 1.0 );
    }
    ...
    if( rcc->b_vbv && rcc->last_satd > 0 )
    {
        ...
        if( bits > rcc->buffer_fill/2 )
            qf = x264_clip3f( (float)( rcc->buffer_fill/(2*bits) ), 0.2, 1.0 );
        q /= qf;
        bits *= qf;
    ...
    else if(rcc->b_2pass)
    {
        ...
        return (double)x264_clip3f((float)q, (float)lmin, (float)lmax);
}
...
static float rate_estimate_qscale(x264_t *h, int pict_type)
{
    ...
    double lmin = rcc->lmin[pict_type];
    double lmax = rcc->lmax[pict_type];
    long total_bits = 8*(h->stat.i_slice_size[SLICE_TYPE_I]
    ...
    if( pict_type == SLICE_TYPE_B )
    {
        ...
        if( h->fref0[0]->i_type == X264_TYPE_BREF )
            q0 -= (float)rcc->pb_offset/2;
        if( h->fref1[0]->i_type == X264_TYPE_BREF )
            q1 -= (float)rcc->pb_offset/2;
        ...
        if(h->fenc->b_kept_as_ref)
            q += (float)rcc->pb_offset/2;
        else
            q += (float)rcc->pb_offset;

        rcc->last_satd = 0;
        return (float)qp2qscale(q);
    }
    else
    {
        ...
        if( rcc->b_2pass )
        {
            ...
            long diff = total_bits - (long)rce.expected_bits;
            q = rce.new_qscale;
            q /= x264_clip3f((float)((abr_buffer - (double)diff) / abr_buffer), .5, 2);
            if( h->fenc->i_frame > 30 )
            {
                /* Adjust quant based on the difference between
                 * achieved and expected bitrate so far */
                double time = (double)h->fenc->i_frame / rcc->num_entries;
                double w = (double)x264_clip3f( (float)time*100, 0.0, 1.0 );
                q *= (float)pow( (double)total_bits / rcc->expected_bits_sum, w );
            }
            q = x264_clip3f( (float)q, (float)lmin, (float)lmax );
        }
        else /* 1pass ABR */
        {
...
//////            rcc->last_satd = x264_rc_analyse_slice( h );
            ...
            else
            {
                ...
                overflow = (double)x264_clip3f( (float)(1.0 + (total_bits - wanted_bits) / abr_buffer), .5, 2 );
                q *= (float)overflow;
            }
            ...
            {
                q = (float)qp2qscale( rcc->accum_p_qp / rcc->accum_p_norm );
                q /= (float)fabs( h->param.rc.f_ip_factor );
            }
            else if( h->i_frame > 0 )
            {
            ...
            {
                q = (float)(qp2qscale( ABR_INIT_QP ) / fabs( h->param.rc.f_ip_factor ));
            }

            //FIXME use get_diff_limited_q() ?
            q = (float)clip_qscale( h, pict_type, q );
        }
    ...
    for( t = 0; t < h->param.i_threads; t++ )
    {
        ...
        if( h->param.i_threads > 1 )
        {
            for( y = rc[t].first_row; y<= rc[t].last_row; y++ )
                rc[t].frame_size_planned += predict_row_size( h, y, (int)qscale2qp(rc[t].qp) );
        }
...
{
    x264_ratecontrol_t *rcc = h->rc;
    unsigned long all_const_bits = 0;
    unsigned long all_available_bits = (unsigned long)(h->param.rc.i_bitrate * 1000 * (double)rcc->num_entries / rcc->fps);
    ...
    for(step = 1E4 * step_mult; step > 1E-7 * step_mult; step *= 0.5){
        ...
        for(i=0; i<rcc->num_entries; i++){
            ratecontrol_entry_t *rce = &rcc->entry[i];
            double bits;
            rce->new_qscale = (float)clip_qscale(h, rce->pict_type, blurred_qscale[i]);
            assert(rce->new_qscale >= 0);
            bits = qscale2bits(rce, rce->new_qscale);

            rce->expected_bits = (unsigned long)expected_bits;
            expected_bits += bits;
            update_vbv(h, (int)bits);
        }

//printf("expected:%llu available:%llu factor:%lf avgQ:%lf\n", (unsigned long)expected_bits, all_available_bits, rate_factor);
