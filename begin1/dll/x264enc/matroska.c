...
struct mk_Writer {
  ...
  long	      def_duration;
  long	      timescale;
  long	      cluster_tc_scaled;
  long	      frame_tc, prev_frame_tc_scaled, max_frame_tc;
...
static int	  mk_writeUInt(mk_Context *c, unsigned id, long ui) 
{
...
static int  	  mk_writeSInt(mk_Context *c, unsigned id, long si) 
{
...
static unsigned	  mk_ebmlSIntSize(long si) 
{
  unsigned char	  c_si[8] = { (unsigned char)si >> 56, (unsigned char)si >> 48, (unsigned char)si >> 40, (unsigned char)si >> 32,
	  (unsigned char)si >> 24, (unsigned char)si >> 16, (unsigned char)si >> 8, (unsigned char)si };
  unsigned	  i = 0;
...
int	  mk_setFrameFlags(mk_Writer *w,long timestamp, int keyframe) {
  if (!w->in_frame)
    return -1;
