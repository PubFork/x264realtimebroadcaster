        ...
        for( idx = 0; idx < 4; idx++ )
        {
            ...
            for( i = 0; i < i_max; i++ )
            {
                ...
                if( i_best > i_satd )
                {
                    ...
                    pels_h = *(unsigned long*)(p_dst_by+7*FDEC_STRIDE);
                    if( !(idx&1) )
                        for( j=0; j<7; j++ )
            ...
            *(unsigned long*)(p_dst_by+7*FDEC_STRIDE) = pels_h;
            if( !(idx&1) )
                for( j=0; j<7; j++ )
...
static void x264_mb_analyse_inter_p8x8_mixed_ref( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        for( i_ref = 0; i_ref <= i_maxref; i_ref++ )
        {
             ...
             m.cost += i_ref_cost;
             i_halfpel_thresh += i_ref_cost;
             *(unsigned long*)a->l0.mvc[i_ref][i+1] = *(unsigned long*)m.mv;
...
static void x264_mb_analyse_inter_p8x8( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    i_mvc = 1;
    *(unsigned long*)mvc[0] = *(unsigned long*)a->l0.me16x16.mv;
    ...
    for( i = 0; i < 4; i++ )
    {
        ...
        *(unsigned long*)mvc[i_mvc] = *(unsigned long*)m->mv;
        i_mvc++;
...
static void x264_mb_analyse_inter_p16x8( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    for( i = 0; i < 2; i++ )
    {
        ...
        for( j = 0; j < i_ref8s; j++ )
        {
             ...
             *(unsigned long*)mvc[0] = *(unsigned long*)a->l0.mvc[i_ref][0];
             *(unsigned long*)mvc[1] = *(unsigned long*)a->l0.mvc[i_ref][2*i+1];
             *(unsigned long*)mvc[2] = *(unsigned long*)a->l0.mvc[i_ref][2*i+2];
...
static void x264_mb_analyse_inter_p8x16( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    for( i = 0; i < 2; i++ )
    {
        ...
        for( j = 0; j < i_ref8s; j++ )
        {
             ...
             *(unsigned long*)mvc[0] = *(unsigned long*)a->l0.mvc[i_ref][0];
             *(unsigned long*)mvc[1] = *(unsigned long*)a->l0.mvc[i_ref][i+1];
             *(unsigned long*)mvc[2] = *(unsigned long*)a->l0.mvc[i_ref][i+3];
...
void x264_macroblock_analyse( x264_t *h )
{
    ...
    int i;
    /* init analysis */
    x264_mb_analyse_init( h, &analysis, x264_ratecontrol_qp( h ) );//对分析进行初始化，设置QP并且避免QP变化太快

    /*--------------------------- Do the analysis ---------------------------*///开始分析
	//判断切片类型：
    if( h->sh.i_type == SLICE_TYPE_I )//如果是I切片
    {
    ...
    else if( h->sh.i_type == SLICE_TYPE_P )//如果是P切片
    {
        int b_skip = 0;
        ...
        else
        {
            ...
            x264_mb_analyse_load_costs( h, &analysis );//分析加载花费

            x264_mb_analyse_inter_p16x16( h, &analysis );//分析帧间16*16模式

            if( h->mb.i_type == P_SKIP )
                return;

            if( flags & X264_ANALYSE_PSUB16x16 )//是否为16*16子块模式
            {//是16*16子块模式则分析帧间P模式8*8子块
                if( h->param.analyse.b_mixed_references )
                    x264_mb_analyse_inter_p8x8_mixed_ref( h, &analysis );
                else
                    x264_mb_analyse_inter_p8x8( h, &analysis );//分析帧间8*8模式
            }
			//分析完16*16和8*8模式后，模式代价值分别保存在analysis.l0.me16x16.cost和analysis.l0.i_cost8x8中

            /* Select best inter mode *///选择最好的帧间模式
            i_type = P_L0;
            i_partition = D_16x16;
            i_cost = analysis.l0.me16x16.cost;

            if( ( flags & X264_ANALYSE_PSUB16x16 ) &&
                analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost )//检测到16*16子块信号并且帧内8*8花费小于16*16运动估计花费
            {
                i_type = P_8x8;
                i_partition = D_8x8;
                ...
                /* Do sub 8x8 */
                if( flags & X264_ANALYSE_PSUB8x8 )//分析帧间8*8子块，进行子宏块分割的判断
                {
                    for( i = 0; i < 4; i++ )//依次对4个子宏块（8*8）进行处理
                    {
                        x264_mb_analyse_inter_p4x4( h, &analysis, i );//此函数实际上是得到4个4*4块的代价和analysis.l0.i_cost4x4[i]
                        if( analysis.l0.i_cost4x4[i] < analysis.l0.me8x8[i].cost )//如果4*4模式优于8*8模式，才进行8*8模式的细分割，代码如下：
                        {
            ...
            /* Now do 16x8/8x16 *///分析帧间16*8/8*16子块
            i_thresh16x8 = analysis.l0.me8x8[1].cost_mv + analysis.l0.me8x8[2].cost_mv;
            if( ( flags & X264_ANALYSE_PSUB16x16 ) &&
                analysis.l0.i_cost8x8 < analysis.l0.me16x16.cost + i_thresh16x8 )//检测16*8和8*16模式
            {
			...
			//帧间模式选择后，对该模式进行亚象素精细搜索。以进一步减少误差。值得注意的是，在前面每个模式的检测时，也要进行亚象素搜索，见x264_me_search_ref()函数的最后几行。这里的亚象素搜索是在前面基础上再进行的精细搜索的。二者亚象素搜索（包括半象素和1/4象素）的次数有subpel_iterations[i][4]确定，看运行帮助-m, --subme <integer>       Subpixel motion estimation and partition实际上subme就是决定模式选择前后亚象素估计的点数，subme越大，压缩效率越好，计算量越大。
			...
			/*最佳帧内宏块模式分析，即分析宏块的帧内编码，包括亮度和色度，并对亮度和色度成分分别附值，
			亮度的16*16、8*8、4*4代价值分别存在
			nalysis.i_satd_i16x16、
			analysis.i_satd_i8x8、 
			analysis.i_satd_i4x4中，
			色度代价值存在analysis.i_satd_i8x8chroma中。*/
            if( h->mb.b_chroma_me )
            {
                ...
                x264_mb_analyse_intra( h, &analysis, i_cost );//分析宏块的帧内编码
    ...
    else if( h->sh.i_type == SLICE_TYPE_B )//如果是B切片
    {
        int i_bskip_cost = COST_MAX;
        int b_skip = 0;

        h->mb.i_type = B_SKIP;
        if( h->mb.b_direct_auto_write )//判断是否是直接模式
        {
        ...
        if( !b_skip )//判断是否是skip模式
        {
            ...
            if( analysis.b_direct_available )
                x264_mb_analyse_inter_direct( h, &analysis );//分析直接模式

            x264_mb_analyse_inter_b16x16( h, &analysis );//分析帧间16*16模式
            ...
            if( flags & X264_ANALYSE_BSUB16x16 )//是否收到16*16子块信号
            {
                x264_mb_analyse_inter_b8x8( h, &analysis );//分析帧内8*8子块
                if( analysis.i_cost8x8bi < i_cost )
                {
                    ...
                    {
                        x264_mb_analyse_inter_b16x8( h, &analysis );//分析帧内16*8子块
                    ...
                    {
                        x264_mb_analyse_inter_b8x16( h, &analysis );//分析帧内8*16子块
