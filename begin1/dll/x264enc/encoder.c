...
static long i_mtime_encode_frame = 0;
static long i_mtime_analyse = 0;
static long i_mtime_encode = 0;
static long i_mtime_write = 0;
static long i_mtime_filter = 0;
/*#define TIMER_START( d ) \
    { \
        long d##start = x264_mdate();
...
static float x264_psnr( long i_sqe, long i_size )
{
...
x264_t *x264_encoder_open   ( x264_param_t *param )
{
    ...
    /* Init x264_t */
    h->out.i_nal = 0;
    h->out.i_bitstream = (int)X264_MAX( 1000000, h->param.i_width * h->param.i_height * 1.7
...
static int x264_slice_write( x264_t *h )
{
    ...
    /* init stats */
    memset( &h->stat.frame, 0, sizeof(h->stat.frame) );//初始化状态参数
    /* Slice */
    x264_nal_start( h, h->i_nal_type, h->i_nal_ref_idc );//NAL单元开始

    /* Slice header */
    x264_slice_header_write( &h->out.bs, &h->sh, h->i_nal_ref_idc );//写切片头部，一些初始化工作
///    if( h->param.b_cabac )//如果是cabac编码，初始化上下文以及编码器
///    {
        /* alignment needed */
///        bs_align_1( &h->out.bs );
...
/*        x264_cabac_context_init( &h->cabac, h->sh.i_type, h->sh.i_qp, h->sh.i_cabac_init_idc );
    ...
    for( mb_xy = h->sh.i_first_mb, i_skip = 0; mb_xy < h->sh.i_last_mb; mb_xy++ )//编码开始，对一个slice中每个宏块进行循环遍历编码，其中const int i_mb_y=mb_xy/h->sps->i_mb_width;和const int i_mb_x=mb_xy%h->sps->i_mb_width;是对宏块位置在slice中的x,y坐标的定位，这个for语句几乎覆盖了整个x264_slices_write()函数
    {
        ...
        /* load cache */
        x264_macroblock_cache_load( h, i_mb_x, i_mb_y );//装载缓冲区，它是将要编码的宏块的周围的宏块的值读进来，要想得到当前宏块的预测值，要先知道上面，左面的预测值。
...
///        TIMER_START( i_mtime_analyse );
        x264_macroblock_analyse( h );//分析宏块参数----重点！！通过一系列的SAD算出最优化方案，
...
///        TIMER_START( i_mtime_encode );
        x264_macroblock_encode( h );//宏块编码，1.判断宏块的类型；2.根据判断的类型进行DCT、量化、ZIGZAG，并记录当前的模式为下次编码宏块（亚宏块）做参考；3.对色度块进行编码，QP限制在0--51之间，选定预测模式；4.求亮度和色度的CBP；5.利用CBP判断要不要SKIP，里面还关系到向量预测。
///        TIMER_STOP( i_mtime_encode );

///        TIMER_START( i_mtime_write );
/*        if( h->param.b_cabac )//是否为cabac编码
        {
            ...
            else
            {
                ...
                x264_macroblock_write_cabac( h, &h->cabac );//若是cabac编码则写入编码cabac
            }
        }
        else
        {*/
            ...
            else
            {
                ...
                x264_macroblock_write_cavlc( h, &h->out.bs );//若不是cabac编码则写入cavlc编码
            }
   ///     }
///        TIMER_STOP( i_mtime_write );
        ...
        /* save cache */
        x264_macroblock_cache_save( h );//保存缓冲区数据，以为下次的预测作为参考
...
///    x264_cpu_restore( h->param.cpu );
        ...
        && h->param.i_scenecut_threshold >= 0 )
    {
        ...
        long i_inter_cost = h->stat.frame.i_inter_cost;
        long i_intra_cost = h->stat.frame.i_intra_cost;

        float f_bias;
        int i_gop_size = h->fenc->i_frame - h->frames.i_last_idr;
        float f_thresh_max = (float)(h->param.i_scenecut_threshold / 100.0);
        ...
        f_bias = (float)X264_MIN( f_bias, 1.0 );
...
{
#ifdef DEBUG_BENCHMARK
    long i_mtime_total = i_mtime_analyse + i_mtime_encode + i_mtime_write + i_mtime_filter + 1;
#endif
    long i_yuv_size = 3 * h->param.i_width * h->param.i_height / 2;
    int i;
    ...
    {
        const long *i_mb_count = h->stat.i_mb_count[SLICE_TYPE_I];
    ...
    {
        const long *i_mb_count = h->stat.i_mb_count[SLICE_TYPE_P];
        const long *i_mb_size = h->stat.i_mb_count_size[SLICE_TYPE_P];
    ...
    {
        const long *i_mb_count = h->stat.i_mb_count[SLICE_TYPE_B];
        const long *i_mb_size = h->stat.i_mb_count_size[SLICE_TYPE_B];
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        if( h->param.analyse.b_transform_8x8 )
        {
            long i_i8x8 = SUM3b( h->stat.i_mb_count, I_8x8 );
            long i_intra = i_i8x8 + SUM3b( h->stat.i_mb_count, I_4x4 )
