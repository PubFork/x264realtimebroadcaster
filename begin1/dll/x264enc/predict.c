...
void x264_predict_8x8_filter( uint8_t *src, uint8_t edge[33], int i_neighbor, int i_filters )
{
    ...
    if( i_filters & MB_TOP )
    {
        ...
        if( i_filters & MB_TOPRIGHT )
        {
            ...
            {
                //*(unsigned long*)(edge+24) = SRC(7,-1) * 0x0101010101010101ULL;
			/*	*(unsigned long*)(edge+24) = SRC(7,-1) * 0x0101010101010101uI64; //lsp060515
                edge[32] = SRC(7,-1);*/
            }
        }
    }
...
{
    const unsigned long top = *(unsigned long*)(edge+16);
    int y;
    for( y = 0; y < 8; y++ )
        *(unsigned long*)(src+y*FDEC_STRIDE) = top;
}
