...
void x264_mb_load_mv_direct8x8( x264_t *h, int idx )
{
    ...
    for( l = 0; l < 2; l++ )
    {
        *(unsigned long*)h->mb.cache.mv[l][x264_scan8[idx*4]] =
        *(unsigned long*)h->mb.cache.direct_mv[l][x264_scan8[idx*4]];
        *(unsigned long*)h->mb.cache.mv[l][x264_scan8[idx*4]+8] =
        *(unsigned long*)h->mb.cache.direct_mv[l][x264_scan8[idx*4]+8];
    }
}
